...
4. The team page is organized into tabs (#274, #444), and lands on Performance: **Performance** (team insights + the Team Pool meter, Team-Lead only; members see a placeholder), **Members** (the roster with letter grades A–F; Team Leads invite designers via an **Invite designer** button that opens a modal), **Design System** and **Writing Style Guide** (team-wide config relocated from personal Settings in #444 — visible to all members, upload/manage gated to Team Leads via `canManage`), and **Reviews** (Beta). **For the MVP launch the Reviews tab is hidden (#302);** restore both Reviews and the member-detail Evaluations tab by flipping `SHOW_EVALUATIONS_AND_REVIEWS` in `src/lib/feature-flags.ts`. The Team Pool box mirrors the dashboard Usage meter — used/limit, monthly reset, hard cap, and a soft-cap tick. The page's top "Team" plan strip was removed (#274; comp-expiry surfacing tracked in #196). **Roster visibility (#343):** every member sees each teammate's **scan count** (activity/volume is not sensitive), but the **average score is manager-only** — it reads as a "how good a designer are you" judgment, so members don't see each other's grades (the average and the team-insights panel are gated on `org:admin` in `/api/dashboard/team`).
...
- Result screen mirrors the web score detail (#386, #392): the score reveals into a card, then tabbed **Findings / Design System / Writing Style Guide / Copy** (Writing Style Guide + Copy lazy-load; Design System renders from scan-time data computed in the sandbox, Team tier only, #400), a per-rung breakdown, and "More Tools" (Accessibility, follow-up) + "Analysis quality" sections. Selecting a frame that was already scored re-displays its result instantly; selecting a fresh frame shows the default state; the button reads "Score" vs "Re-score this screen".
