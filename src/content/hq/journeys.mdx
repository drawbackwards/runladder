...
The plugin source lives in `drawbackwards/ai-design-assistant`. Plugin backend endpoints live in `runladder` under `/api/plugin/*`. Current bundle version is 1.14.0 (`PLUGIN_VERSION` in `plugin/ui.html`; latest-available mirrored in runladder's `src/lib/plugin-version.ts`, #414).
