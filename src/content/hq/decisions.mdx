...
**Rule:** the Q4 Design System Compliance feature ([#400](https://github.com/drawbackwards/runladder/issues/400)) ingests the client's **Figma** library/variables/components and diffs a scored frame against them — Figma only for the initial build. The earlier tiered proposal (brand-guideline PDF + tokens-file ingestion) is dropped; revisit other sources later. Open question, unresolved: whether compliance affects the numeric score or is advisory-only — ties to the SOW §2.4 wording ("scoring is calibrated to reflect them"); the standing principle is that user-provided guidelines must never move the Ladder score, so advisory-only is the likely answer, pending confirmation before the SOW is signed.
