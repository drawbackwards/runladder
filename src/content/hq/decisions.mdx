...
**Rule:** On upload, detect internal contradictions/ambiguities and **surface them** on Settings → Style Guide — each with a plain-language summary and **how Ladder will interpret it** (the compliance engine applies the MOST SPECIFIC rule for an element; a rule naming "field labels" beats a general "all content" rule). We do **not** let the team resolve the conflict in the app (no pick-one toggle); the guidance is "edit the guide and upload a new version." This keeps scope tight, keeps the guide as the single source of truth, and turns a weakness (can't decide) into a feature (we caught a flaw in your guide). Detection is a best-effort pass at upload; it never blocks. The compliance prompt itself is precision-tuned so it never over-flags compliant copy regardless ([#362](https://github.com/drawbackwards/runladder/issues/362)).
