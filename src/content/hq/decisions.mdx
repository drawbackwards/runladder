...
## Clerk stays on Free until a paying prospect; upgrade path is Pro, then the B2B add-on (2026-08-11)

**Rule:** stay on Clerk's **Free** plan for now. Upgrade to **Pro ($25/mo, or $20/mo billed annually)** at the **first paying prospect**, specifically right before we send Clerk invite or verification emails to a real prospect or provision the first client (Michael, 2026-08-11; [#255](https://github.com/drawbackwards/runladder/issues/255)). Pro is the tier that unlocks custom email templates and removes Clerk branding, which fixes the off-brand Invitation and Verification-code emails ([#256](https://github.com/drawbackwards/runladder/issues/256)). Add the **B2B Authentication add-on (about $85/mo billed annually; 100 organizations included, then $1/org/mo)** only when a client actually needs it: more than 20 members in one org, verified or restricted domains, or custom roles beyond `org:admin` / `org:member`. That add-on removes the free plan's 20-member-per-org cap ([#254](https://github.com/drawbackwards/runladder/issues/254)), which is the ceiling behind the "seats are not capped" decision below.

**Why:** the only Clerk limit that is an active problem today is the off-brand emails, and nothing paying is on the platform yet, so there is no reason to pay before a real prospect sees them. Pro alone fixes that for $25/mo. The bigger B2B features (unlimited membership, verified domains, custom roles) each require the ~$85/mo add-on, and none are needed until a specific client triggers them, so they stay event-gated rather than pre-paid. Business ($300/mo) buys nothing we need. Clerk restructured plans in Feb 2026, folding the old "Enhanced Authentication" add-on into Pro, which is why custom email templates now sit in the $25 tier rather than a separate purchase.

**Owner:** Ward holds the Clerk account and billing. Michael runs day-to-day operations but is an employee, so account ownership should sit with a principal; this is reassignable later, since the Clerk account owner and payment method are just settings.

**How to apply:** when the first prospect provisioning is scheduled, Ward upgrades to Pro in Clerk billing. Add the B2B add-on the first time a client crosses 20 designers or needs domain or role features.
...
**Rule:** a screenshot thumbnail is stored as a **private** Vercel Blob with only a pointer key in Redis, never embedded in the `user:{id}:scores` entry (Michael, 2026-08-06; [#442](https://github.com/drawbackwards/runladder/issues/442)). **Why:** entries glued the base64 image inline, so history payload scaled with image weight, not score count — one active account crossed Upstash's 10MB request ceiling and 500'd the whole dashboard on 2026-08-05. Every full-history reader (dashboard, team/admin rollups, the lifecycle purge) paid to move images most of them discard.

**Why Blob over per-score Redis keys (the alternative Chester floated):** images don't belong in our hot datastore — Blob is purpose-built and cheap, and it yields a real per-thumbnail URL, which is the foundation the dashboard needs to paginate + lazy-load the score list ([#391](https://github.com/drawbackwards/runladder/issues/391), phase 2). Private access + an auth-gated proxy route (`/api/dashboard/scores/[id]/thumbnail`, same `resolveScoreOwner` gate as the score detail) preserve the existing privacy model: a paid user's screenshot is never at a guessable public URL. Pagination/lazy-load is deliberately a **fast-follow**, not part of the outage fix, to keep the cure small and shippable for the Lumin contract.

## Releases are tracked on the GitHub Project "Released" view; Release Date is stamped at deploy (2026-07-31)

**Rule:** the org **"Ladder" GitHub Project** (project 5) has a **Release Date** field (Michael, 2026-07-30): setting it drops a ticket off the active board into the **Released** view, giving each push to prod a recorded manifest. Process: when a prod deploy goes out, stamp Release Date = deploy date on every ticket that deploy shipped (issues closed by PRs merged since the prior deploy). Sara stamps automatically during the ship flow, alongside the existing close-issues-as-we-ship habit; commands are in CLAUDE.md → Git Workflow → Release stamping. Backfill: done 2026-08-04 — all 107 Done items (Michael's estimate was ~51) stamped with the flat date **2026-07-31** per Chester + Michael; a few tickets that actually went live in the 2026-08-04 deploy knowingly carry the earlier date. Per-deploy accuracy starts from the next push.

## The learning-store industry taxonomy is locked; slugs never change, categories are add-only (2026-07-31)

**Rule:** the 16-category client-industry list in `src/lib/industries.ts` is approved for launch (Chester + Michael, Slack, 2026-07-31). Industry is the only client-linked dimension in the de-identified learning store ([#422](https://github.com/drawbackwards/runladder/issues/422)), stamped into records at capture/purge time — records can never be re-bucketed afterwards, so the taxonomy is deliberately conservative. It is intentionally NOT the Top 100's category list (those are product categories, this is what business a client is in); where the two overlap, the vocabulary stays aligned (Fintech, E-commerce, Healthcare).

**How to change it:**
- **Add a category:** self-serve — "Add new industry" in the admin dropdowns (Add Team Client form, Team Detail → Scoring Data). Additions are slugified, deduped, and converge onto near-matches ("Fintech" lands on Fintech & Banking, never a parallel bucket). Durable additions should graduate into the `INDUSTRIES` list in code with the SAME slug.
- **Reword a label:** edit the `label` in `src/lib/industries.ts` — labels are display-only and always safe to change.
- **Never rename or delete a slug** (`value`): the aggregate keys off it, and renaming forks the data. Retiring a mistaken slug is a deliberate code + Redis operation with a data migration, not a UI action.
- Reserved buckets `internal` (Drawbackwards dogfood data) and `unknown` (no-org individuals) are capture-time defaults, never pickable or addable. Making internal scores industry-attributable is [#429](https://github.com/drawbackwards/runladder/issues/429).

## Design System Compliance is advisory-only; v1 auto-reads enabled libraries; two distinctly-named compliance tabs (2026-07-31)

**Rule 1 — advisory-only, resolved:** Design System Compliance ([#400](https://github.com/drawbackwards/runladder/issues/400)) NEVER affects the Ladder score, same invariant as the Writing Style Guide (see the blockquote in [Architecture → Shared services](/hq/architecture)). This resolves the open question in the 2026-07-30 entry below. Chester flagged the SOW §2.4 wording ("scoring is calibrated to reflect them") to Ward; the SOW is with the client awaiting signature and no revision is expected — the product behavior stands regardless: client-provided guidelines must never move the number, because the score's value is its consistency across everyone.

**Rule 2 — v1 ingestion is zero-setup (Phase 1, auto-read):** the plugin diffs a scored frame against the libraries **enabled in that file**, read in the sandbox at scan time. Chosen over an explicit library-connect flow because it works on ANY Figma plan (many prospects are not on Enterprise; Figma's Variables REST API is Enterprise-only), requires no tokens or settings UI, and inherently tolerates incomplete libraries. The connect flow (Phase 2) is the designed enhancement, not the default — both phases produce the same "library snapshot" artifact, so the diff engine carries over unchanged. Full tier analysis + the seven-candidate drift-check list (v1 ships 3 + 1 stretch) are recorded on [#400](https://github.com/drawbackwards/runladder/issues/400).

**Rule 3 — naming:** the two compliance features are always presented as two distinctly-named surfaces — **"Design System"** and **"Writing Style Guide"** (settings tabs, plugin result tabs, score-detail sections) — so nobody confuses copy compliance with design-token compliance. The bare label "Style Guide" is retired.

**Rule 4 — surface scope:** detection runs ONLY in the Figma plugin (drift facts are properties of Figma layers; a screenshot-based guess would violate the confidence-labeling rule). Findings persist with the score so the web dashboard displays them. Tighter than the Writing Style Guide's all-surface best-effort model, deliberately.

## Team seats are NOT capped — allow overage, have a sales conversation (2026-07-30)

**Rule:** don't cap Team membership or enforce the SOW's "5 designers + 1 lead." Let clients add as many designers as they want; when they exceed the contract, that's a sales conversation, not a block. Crucially, **do not show "seats remaining" anywhere in the UI** — a visible limit would discourage clients from adding designers, and Ward wants them adding ~a dozen. Seat usage is monitored manually via the admin Team Detail roster ([#397](https://github.com/drawbackwards/runladder/issues/397)); the built seat-cap/buy-seats work ([#296](https://github.com/drawbackwards/runladder/issues/296)) was closed and discarded. **Constraint to watch:** Clerk's free-plan 20-member-per-org cap ([#254](https://github.com/drawbackwards/runladder/issues/254)) becomes the real ceiling — not urgent (no client is near 19 designers yet), addressed by the Clerk paid upgrade ([#425](https://github.com/drawbackwards/runladder/issues/425)).

## Design System Compliance supports Figma design systems only (for now) (2026-07-30)

**Rule:** the Q4 Design System Compliance feature ([#400](https://github.com/drawbackwards/runladder/issues/400)) ingests the client's **Figma** library/variables/components and diffs a scored frame against them — Figma only for the initial build. The earlier tiered proposal (brand-guideline PDF + tokens-file ingestion) is dropped; revisit other sources later. The score-impact question raised here was **resolved 2026-07-31: advisory-only** — see the entry above.

## Ladder learns from customer screens via industry-anonymized, categorized findings (2026-07-30)

**Rule:** the "de-identified aggregate" in the contract-termination lifecycle ([#398](https://github.com/drawbackwards/runladder/issues/398)) is how Ladder improves over time — customer screens teach the engine, but customer-specific data is stripped. **Anonymize by industry** and **categorize findings** so they can be applied to future scans. This is the same learning system as [#422](https://github.com/drawbackwards/runladder/issues/422) (structure/categorize scoring data for later insights) — design them together.
...
## Ambiguous style guides: surface the conflict, don't resolve it in-app (2026-06-29)

**Context:** A client's uploaded guide contradicted itself — "use title case across all content" AND "capitalize the first word of field labels" (title case vs. sentence case for the same element). No checker can be reliably "right" when the source disagrees with itself, and silently guessing an interpretation is dangerous for a tool we sell as *enforcement*.

**Rule:** On upload, detect internal contradictions/ambiguities and **surface them** on Settings → Style Guide — each with a plain-language summary and **how Ladder will interpret it** (the resolution: the MOST SPECIFIC rule for an element wins; a rule naming "field labels" beats a general "all content" rule). We do **not** let the team resolve the conflict in the app (no pick-one toggle); the guidance is "edit the guide and upload a new version." This keeps scope tight, keeps the guide as the single source of truth, and turns a weakness (can't decide) into a feature (we caught a flaw in your guide). Detection is a best-effort pass at upload; it never blocks.

**Update (PR #376):** those resolutions are now **ENFORCED, not just displayed.** We learned the compliance model can't reliably resolve a self-contradictory guide from the prompt alone (it flip-flops), and the prompt must stay generic (no guide-specific examples) to work for any guide. So the detected resolution is injected into the compliance ruleset as an authoritative "RESOLVED CONFLICTS" section — making the contradictory case deterministic without overfitting the prompt. The compliance prompt biases toward precision (never over-flag compliant copy) regardless ([#362](https://github.com/drawbackwards/runladder/issues/362)).

**How to apply:** when a team reports "wrong" style findings, first check Settings → Style Guide for surfaced ambiguities and inspect their distilled ruleset — the fix is often clarifying their guide, not the engine.

---

## Canonical scoring input is ground-truth text where we have it; image is best-effort (2026-06-29)

**Context:** Scoring the same screen on web vs. the Figma plugin returned different results ([#362](https://github.com/drawbackwards/runladder/issues/362)). Investigating corrected a long-held belief (Ward's "web and the plugin score from the same API"): **they never did for scoring.** The plugin shipped its own engine in March 2026; #75 (Apr 2026) built `/api/plugin/analyze` and migrated the plugin's *service* endpoints to `runladder`, but the *scoring computation* was never migrated and nothing ever called that endpoint. The old `/hq/architecture` "one engine for web, plugin, and Skill" line described #75's intent, not reality.

**Rule:** The real divergence is (1) two forked prompts, (2) style-compliance logic implemented twice, and (3) input fidelity. On input, the divide is **source type, not surface**: the Figma frame and a URL's DOM both give ground-truth text; a raw image upload gives only pixels. So the engine treats **exact on-screen text as canonical when available** (Figma layers, URL DOM) and falls back to reading the image (best-effort) otherwise. Style compliance is the most text-sensitive operation, so for the **style-guide feature specifically**: full guarantee on Figma + URL; image uploads run best-effort and show a gold caveat ("score from Figma or a URL for a complete check"). We do **not** pretend pixel inference equals ground truth — that false confidence is what let the web miss violations the plugin caught.

**Consequence:** Consolidation is phased. Chunk 1 (PR #366) put style/copy checking behind one canonical engine fed ground-truth text. Chunk 2 unifies the Ladder score itself. The numeric score stays image-based for now (engine v1.2 unchanged); only the advisory style/copy pass became text-driven.

---

## Team style guide is advisory — it never affects the Ladder score (2026-06-24)

**Rule:** A Team-plan team lead can upload a PDF writing style guide (Settings → Style Guide). Ladder flags on-screen copy that deviates from it — on web scores now, the Figma plugin's Improve Copy next — but style-guide compliance is **advisory only and never changes the numeric Ladder score**. It runs as a SEPARATE, parallel model pass (Sonnet) alongside scoring (Haiku); the score is never affected. The result carries a **status** so the UI is never silent when a guide exists: `compliant` (positive "Complies with {team} style guide" confirmation), `issues` (findings list), or `unavailable` (the pass errored — shown as a calm note, score still unaffected).

**Storage:** the original PDF lives in Vercel Blob; on upload we distill it once (Claude reads the PDF natively — no PDF parser) into a compact ruleset cached in Redis (`org:{orgId}:style-guide`, JSON: ruleset + team name) for cheap per-score checks. Gated to the Team plan; uploads require the team lead (`org:admin`); the PDF is served only through an authed download route, never a public Blob URL. Phase 1 (web) shipped; the Figma plugin integration is phased as a follow-up (it needs org context plumbed into the plugin auth).
