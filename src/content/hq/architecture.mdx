...
> **The Ladder score and the two compliance features (Writing Style Guide, Design System) are separate features. Compliance findings NEVER affect a screen's Ladder score — not by a point, not by a decimal, ever.** The Ladder score measures user experience (hierarchy, clarity, flow, assistance). Writing Style Guide compliance is an advisory, team-specific check of on-screen copy against an uploaded brand/writing guide (separate model pass, `analyzeStyleCompliance`). Design System Compliance ([#400](https://github.com/drawbackwards/runladder/issues/400)) is an advisory, deterministic diff of a scored Figma frame against the design-system libraries enabled in that file (computed in the plugin sandbox, no model call). Both are attached to the result for display only and computed independently of the number. A screen's Ladder score is identical whether or not the team has a guide or a library. This separation is a hard invariant — client-provided guidelines must never move the score (see the SOW §2.4 discussion in the Decisions Log) — any change that lets compliance influence the score is a bug.

**Design System Compliance ([#400](https://github.com/drawbackwards/runladder/issues/400)).** v1 ingestion is Phase 1 (auto-read): the plugin reads the libraries enabled in the scored file (`figma.teamLibrary` variable collections + per-node style/variable bindings, `teamlibrary` manifest permission) and runs four deterministic checks in one frame walk — detached instances (`detachedInfo`), locally rebuilt components (local main component while a library is enabled), colors not sourced from the library (raw values, local paint styles, and local variables all flag — the library is the single source of truth; a near-match token suggestion is labeled estimate), and text not bound to a remote text style. The library snapshot is session-cached in the sandbox. Findings + a small library summary ride the persist-score payload (`designSystem` field, capped at 50 findings server-side) and render in the plugin's Design System tab and the dashboard score detail. Team tier only. The designed upgrade path (Phase 2, not built): a Team-Lead library-connect flow syncing a server-side snapshot per org — enables whole-library checks on any Figma plan and, on Figma Enterprise only, server-side variables via REST. Full candidate-check list and tier analysis live on [#400](https://github.com/drawbackwards/runladder/issues/400).
...
**Where the Redis databases actually live (learned the hard way, 2026-08-03).** The runladder databases (`runladder-prod` on AWS us-east-1, plus the dev instance) are **Vercel Marketplace resources** — they exist in an Upstash account auto-linked to the drawbackwards Vercel team, NOT in anyone's personal Upstash account (Ward's personal console contains nothing Ladder-related; don't look there). Console access: Vercel → drawbackwards team → **Storage** → select the store → **"Open in Upstash"** (SSO). Anyone on the Vercel team has full console access — there is no separate Upstash login to transfer or request. Creating/managing these databases happens exclusively via Vercel (Upstash says so in-console). Credentials: Vercel Storage → "Show secret" (never eyeball Sensitive env vars in the Vercel env UI — they read back blank). The prod database runs pay-as-you-go without Upstash's "Prod Pack" add-on (multi-zone replication, uptime SLA, extended backups) — deliberately not purchased in passing; that upgrade is an owner decision.
...
**Data lifecycle + learning store ([#398](https://github.com/drawbackwards/runladder/issues/398)/[#422](https://github.com/drawbackwards/runladder/issues/422)).** Two halves of one system (full design: `docs/data-lifecycle-and-learning.md`). The **learning store** (`src/lib/learning.ts`) is the de-identified aggregate the SOW lets us retain indefinitely: every persisted score is projected — allowlist-only, one-way — into `learn:*` Redis keys (raw monthly record lists + per-industry rollups of scores, finding categories, rungs, and uplift). Nothing identifying survives: no user/org ids, screen names, finding text, thumbnails, or exact timestamps; industry (org `publicMetadata.industry`, set on the admin Team Detail) is the only client-linked dimension. The **termination lifecycle** rides on it: an admin marks an org `terminated`, and a daily Vercel cron (`/api/cron/data-lifecycle`, `CRON_SECRET`-gated) purges the org 30 days later — replaying each member's history through the learning projection, then deleting every per-user and per-org Customer Content key (and the style-guide Blob, plus every externalized score-thumbnail Blob — #442). Admin org DELETE cascades the same purge. Members also in another org are skipped and reported for manual review.

**Score storage: thumbnails are externalized ([#442](https://github.com/drawbackwards/runladder/issues/442)).** A user's score history is a Redis zset (`user:{id}:scores`, timestamp-scored JSON members). Thumbnails are NOT inline — `persistScoreEntry` (`src/lib/scores.ts`) offloads each screenshot to a **private** Vercel Blob (`score-thumbs/{userId}/{scoreId}`, `addRandomSuffix`) and stores only a pointer key (`user:{id}:thumb:{scoreId}` → blob URL) plus `hasThumbnail: true` on the entry. Readers hand the client a proxy URL (`withThumbnailUrl`) served by the auth-gated `/api/dashboard/scores/[id]/thumbnail` route; the blob is never fetchable directly. This keeps full-history reads (dashboard, team/admin rollups, the lifecycle purge) small — an inline thumbnail once pushed one heavy account past Upstash's 10MB request ceiling and 500'd the dashboard on 2026-08-05 ([#441](https://github.com/drawbackwards/runladder/issues/441) chunked the reads as a tourniquet; #442 is the cure). Legacy entries still carrying inline bytes are read through untouched until `scripts/backfill-score-thumbnails.mjs` migrates them.

**Token-cost accounting (COGS, internal only).** Every Anthropic call in this repo routes its response `usage` through the single chokepoint `recordTokenCost(...)` in `src/lib/token-cost.ts`, which prices tokens per model and accumulates cost per **user, per month, per category** in the Redis hash `usage:cost:{userId}:{yyyymm}` (micro-USD, no TTL). Categories: `score`, `overhead` (moderation + transcription), `copy`, `a11y`, `style-guide`, `design-system` (reserved — v1 is deterministic and spends $0; wired so any future model-assisted check is visible from day one, [#400](https://github.com/drawbackwards/runladder/issues/400)), `annotation`, `chat`, `feedback`. The admin Team Detail page ([#397](https://github.com/drawbackwards/runladder/issues/397)) sums current members' costs to show what a client costs us to run, with a per-category popover ([#406](https://github.com/drawbackwards/runladder/issues/406)). Months before this shipped have no token data, so they're shown as an **estimate** (score count × a per-score rate; score category only) and tagged "Estimated" in the UI. This number is **internal COGS only — never client-facing**, and it is intentionally decoupled from the user-facing "scores this month" count (bonus plugin features cost money but are not scores). Pricing rates live in `RATES` in `token-cost.ts`; update them when Anthropic pricing changes.
