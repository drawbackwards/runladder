...
| `/api/stripe/checkout` | POST | Clerk session | Pro upgrade Stripe checkout session (returns `{ url }`). |
| `/api/stripe/checkout` | GET | Clerk session | Creates Pro checkout session and 302-redirects to Stripe. Used as Clerk post-auth redirect target (`?plan=pro`). |
