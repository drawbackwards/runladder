...
| `/api/dashboard/scores` | GET | Clerk session | User's score history. Each entry's `thumbnail` is a proxy URL (below), not inline bytes (#442). |
...
| `/api/dashboard/scores/[id]/thumbnail` | GET | Clerk session | Auth-gated proxy for a score's screenshot, externalized to a private Vercel Blob (#442). Streams the image bytes; reachable by exactly the same people as the score itself (owner, or a Team Lead via `?member=<userId>` — shared `resolveScoreOwner`). Optional `?size=sm` returns a small (~96px) variant resized on read for list rows (#448); omit for the full-resolution detail image. |
...
| `/api/plugin/persist-score` | POST | Service token | Save a plugin-originated score (server-to-server). Body may include `score.designSystem` — the plugin-computed Design System Compliance advisory payload (#400): `{ status: "compliant" \| "issues" \| "no-library" \| "unavailable", library: { names, collections, colorVariables }, findings: [{ check, node, detail, expected?, actual?, confidence }] }`, findings capped at 50 server-side. Advisory only — never affects the numeric score. |
