...
| `/api/dashboard/scores/[id]/thumbnail` | GET | Clerk session | Auth-gated proxy for a score's screenshot, externalized to a private Vercel Blob (#442). Streams the image bytes; reachable by exactly the same people as the score itself (owner, or a Team Lead via `?member=<userId>` — shared `resolveScoreOwner`). |
