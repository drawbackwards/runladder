...
| Dashboard score list windows the render (~30 rows, reveals more on scroll via IntersectionObserver) and list-row thumbnails load a small ~96px variant instead of the full 1400px image (#448, follow-up to #442). The full array stays client-side for the activity heatmap; server-side pagination is deferred. | All signed-in | Live (#448) | `src/app/dashboard/page.tsx`, `src/hooks/use-infinite-window.ts`, `src/app/api/dashboard/scores/[id]/thumbnail/route.ts` (`?size=sm`), `src/lib/scores.ts` (`scoreThumbnailUrl`) |
