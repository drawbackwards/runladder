...
| Feature | Roles | Status | Evidence |
| --- | --- | --- | --- |
| Public scoring tool (`/score`). **Deterministic (PR #377, engine v1.3):** scoring runs at temperature 0 and the score is cached content-addressed by model+prompt+image, so the same screenshot always returns the same score (#210/#343); re-scores are sub-200ms cache hits. (Same-input only; cross-surface parity is bounded by input fidelity.) | All signed-in | Live | `src/app/score/`, `src/lib/scoring.ts` |
