...
| Personal analytics (#467, Phase A): the dashboard has **Overview / Analytics** tabs; the Analytics tab reads score history client-side into charts (score-over-time with a 0-5 axis, rung profile, top recurring finding categories, portfolio distribution, plus a stat row) and filters them by **Industry** and **Tags** (the #429 tagging payoff), each a single-select dropdown. No new endpoints. Team-wide analytics, auto-suggested industry, and bulk tagging are later #467 phases. | Pro+ / Team | Live (#467 Phase A) | `src/components/dashboard/DashboardAnalytics.tsx`, `src/app/dashboard/page.tsx` |
| Dashboard score list windows the render (~30 rows, reveals more on scroll via IntersectionObserver) and list-row thumbnails load a small ~96px variant instead of the full 1400px image (#448, follow-up to #442). The full array stays client-side for the activity heatmap; server-side pagination is deferred. **List / grid toggle (#471):** a header control switches between the list and a grid of cards with a larger 640px `md` thumbnail variant (resized on read + cached); defaults to grid (#473); the choice persists in localStorage. | All signed-in | Live (#448, #471) | `src/app/dashboard/page.tsx`, `src/hooks/use-infinite-window.ts`, `src/app/api/dashboard/scores/[id]/thumbnail/route.ts` (`?size=sm\|md`), `src/lib/scores.ts` (`scoreThumbnailUrl`) |
...
| Rename a scored screen (#469): a pencil next to the screen name on the score detail page opens an inline editor (Enter saves, Esc cancels). Owner-only; preserves the surface suffix so the Web/Figma/Skill tag stays correct. Persists via the owner-scoped tags PATCH, extended to accept `screenName`. | All signed-in (own scores) | Live (#469) | `src/app/dashboard/scores/[id]/page.tsx`, `src/app/api/dashboard/scores/[id]/tags/route.ts` |
...
| Design System Compliance (#400): at scan time the plugin diffs the frame against the design-system libraries **enabled in the scored file** (zero setup, works on any Figma plan) and flags drift — detached instances, locally rebuilt components, colors not sourced from the library (raw values, local styles, local variables), text not on a library text style. Deterministic (no LLM calls, $0 COGS); findings labeled exact vs estimate; `no-library` is a first-class state. Shown in the plugin's Design System tab and persisted with the score for the dashboard section. Advisory only, never affects the score. The Design System + Writing Style Guide config surfaces live on the **Team page** (`/dashboard/team`, Design System / Writing Style Guide tabs, relocated from Settings in #444; visible to all members, manage gated to Team Leads). Future: Team-Lead library connect (whole-library checks, #461 Phase 2) manages from the Team page → Design System tab | Team | Live (plugin 1.14.0) | `ai-design-assistant` `plugin/code.js` (compliance pass), `plugin/manifest.json` (`teamlibrary` permission), `src/components/DesignSystemCompliance.tsx`, `src/app/api/plugin/persist-score/route.ts`, `src/components/team/OrgConfigCards.tsx`, `src/app/dashboard/team/page.tsx` |
...
| De-identified learning store (#422): every score is projected (allowlist, one-way) into `learn:*` keys — categorical/numeric facts by industry + month, no ids/text/pixels — retained independently of any deletion clock (SOW 6.4). Org `industry` comes from a dropdown (curated base list `src/lib/industries.ts` + add-only admin registry `src/lib/industry-registry.ts` via "Add new industry" — slugified, deduped, no rename/delete, stable slugs; no "Other" bucket by design), required on the Add Team Client form and editable on Team Detail. Live capture at persist time + backfill at termination. Multi-industry accounts (agencies, #429) tag industry per score instead of inheriting one org industry, so their live capture is DEFERRED until a score is tagged (records are immutable and can never be re-bucketed); see the Per-score tagging row. | Admin (internal data) | Live (PR #432) | `src/lib/learning.ts` (+ `learning.test.ts`), `src/lib/scores.ts`, Team Detail industry field |
| Per-score tagging (#429): on multi-industry accounts (agencies + consultancies with `industryMode: "multiple"`, individual Pro, and the internal Drawbackwards org) the score owner tags each score's **industry** (controlled taxonomy; single-industry accounts inherit the org industry read-only) plus free-form **tags**, from the score detail page. Industry feeds the de-identified learning store (deferred capture, see the learning-store row); free-form tags stay private to the account and never enter the store. Account mode is set by admins via a "Multiple / Agency" option in the industry picker on the Add Team Client form + Team Detail. Analytics (by-industry charts), auto-suggested industry, and bulk tagging are Phase 2. | Pro+ / Team | Live (#429 Phase 1) | `src/components/ScoreTags.tsx`, `src/app/api/dashboard/scores/[id]/tags/route.ts`, `src/app/api/industries/route.ts`, `src/lib/learning.ts`, `src/lib/orgs.ts` (`isMultiIndustryOrg`), `src/components/admin/IndustrySelect.tsx` |
