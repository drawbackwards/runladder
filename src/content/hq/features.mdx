...
| Team style guide compliance: Team Lead uploads a PDF writing style guide (Settings → Style Guide); Ladder flags on-screen copy that deviates from it on web scores, with a suggested fix — advisory, never affects the score. Figma plugin integration is phased (follow-up). | Team Lead (upload) / Team (sees findings) | Live (PR #360) | `src/app/settings/page.tsx`, `src/app/api/org/style-guide/`, `src/lib/style-guide.ts`, `src/lib/scoring.ts` |
