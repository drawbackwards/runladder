...
import { TEARDOWN_POSTS, getTeardownBySlug } from "../posts";

type Props = { params: Promise<{ slug: string }> };

export async function generateStaticParams() {
  return TEARDOWN_POSTS.map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const post = getTeardownBySlug(slug);
  if (!post) return {};
  return {
...
function renderContent(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h3 key={i} className="text-base font-bold text-foreground mt-8 mb-3">
          {line.slice(4)}
        </h3>
      );
      i++;
...
export default async function TeardownPage({ params }: Props) {
  ...
  const currentIndex = TEARDOWN_POSTS.findIndex((p) => p.slug === slug);
  const prev =
    currentIndex < TEARDOWN_POSTS.length - 1
      ? TEARDOWN_POSTS[currentIndex + 1]
      : null;
  const next = currentIndex > 0 ? TEARDOWN_POSTS[currentIndex - 1] : null;
    ...
    <div className="pt-32 pb-24 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-10 flex items-center gap-2 text-xs text-muted">
          <Link href="/teardowns" className="hover:text-body transition-colors">
            Teardowns
          </Link>
          <span>/</span>
          <span className="text-body truncate">{post.product}</span>
        </div>
        ...
        <div className="mb-10">
          <div className="flex items-start justify-between gap-6 mb-6">
            ...
            <div className="flex flex-col items-end flex-shrink-0">
              ...
              {post.scoreTbd ? (
                <span className="font-mono text-[9px] text-muted uppercase tracking-widest mt-1">
                  score pending
                </span>
              ) : (
                <span className="font-mono text-[9px] text-muted uppercase tracking-widest mt-1">
          ...
          <p className="text-base text-body leading-relaxed">{post.subtitle}</p>
        </div>
          ...
          <div className="flex items-center gap-3">
            <a
              href={`https://x.com/intent/tweet?text=${encodeURIComponent(
                ...
                `https://runladder.com/teardowns/${post.slug}`
              )}`}
              target="_blank"
