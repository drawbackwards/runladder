...
export default function AdminPage() {
  ...
  async function refresh() {
    ...
    try {
      ...
      if (invRes.status === 403 || usrRes.status === 403) {
        // Should not happen: the admin layout gates access before this page
        // mounts. If it does (race / token expiry), silently bail.
        return;
      }
  ...
  // Auth + access gating live in the admin layout (#231); this page only
  // renders its tab content.
          ...
          <div className="border border-[#333] bg-[#1e1e1e] p-4 mb-3">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <button
                onClick={generate}
                disabled={generating}
                className="ml-auto text-xs font-semibold bg-ladder-green text-[#1a1a1a] uppercase tracking-widest px-5 py-2.5 hover:bg-ladder-green-light transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
