...
/**
 * Composable Reviews building blocks, shared between the dedicated
 * `/dashboard/reviews` route and the team dashboard's Reviews tab so the two
 * never drift. Presentational only; seeded from MOCK_REVIEWS until the GA build
 * wires real persistence.
 */

/** Intro blurb + the "New review" action. */
export function ReviewsIntro() {
  return (
    <div className="mb-8 flex items-end justify-between gap-6 flex-wrap">
      <p className="text-sm text-muted font-sans max-w-xl leading-relaxed">
        Group iterations of a design into a Review. Watch scores evolve. Invite
        peers to drop notes on the screen and weigh in with a Team Take alongside
        the Ladder score.
      </p>
      <button
        type="button"
        className="text-[11px] font-semibold uppercase tracking-widest bg-ladder-green text-[#1a1a1a] px-5 py-2.5 hover:bg-ladder-green-light transition-colors"
      >
        + New review
      </button>
    </div>
  ...
  return (
    <div className="grid grid-cols-3 gap-2 mb-8">
      ...
      <StatCard
        ...
        value={reviews.reduce((sum, r) => sum + r.frames.length, 0).toString()}
      />
      <StatCard
        ...
        value={reviews
          .reduce(
            (sum, r) =>
              sum + r.frames.reduce((s, f) => s + f.teamTakes.length, 0),
            0,
          )
          .toString()}
      />
...
/**
 * Eyebrow + the reviews list. `showViewAll` adds a "View all reviews" text link
 * below the list (used on the team tab, which links into the full route).
 */
export function ActiveReviewsList({
  ...
  return (
    <section>
      <SectionLabel className="mb-3">Active reviews</SectionLabel>
            ...
            >
              ...
              <div className="flex items-end gap-8 flex-wrap">
                ...
                <div className="ml-auto text-right space-y-1">
                  ...
                  <p className="text-[10px] text-muted font-sans">
                    <span className="tabular-nums">{roll.totalPins}</span> pins ·{" "}
                    <span className="tabular-nums">{roll.totalTakes}</span> team
                    takes
                  </p>
                  <p className="text-[10px] text-muted font-sans">
